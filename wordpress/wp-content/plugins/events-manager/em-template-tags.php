...
function em_events_admin($args = array()){
	...
	if( is_user_logged_in() && current_user_can('edit_events') ){
     ...
     //       $owner_events = $EM_Person->get_events();
     //       $ff = $owner_events[0];
      //      $dd = $ff->location_id;
			$argst = array('order' => $order, 'search' => $search, 'owner' => get_current_user_id());// by jwn
            $EM_Eventst = EM_Events::get( $argst );
            $dd = $EM_Eventst[0]->location_id;

            $args = array('order' => $order, 'search' => $search, 'location_id' => $dd);
