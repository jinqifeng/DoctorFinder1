<?php
	...
	<div class="em-events-admin-list">
		<?php
			echo $EM_Notices;
			//if(!empty($show_add_new) && current_user_can('edit_events')) echo '<a class="em-button button add-new-h2" href="'.em_add_get_params($_SERVER['REQUEST_URI'],array('action'=>'edit','scope'=>null,'status'=>null,'event_id'=>null, 'success'=>null)).'">'.__('Add New','events-manager').'</a>'; // by jwn
            if(!empty($show_add_new) && current_user_can('edit_users')) echo '<a class="em-button button add-new-h2" href="'.em_add_get_params($_SERVER['REQUEST_URI'],array('action'=>'edit','scope'=>null,'status'=>null,'event_id'=>null, 'success'=>null)).'">'.__('Add New','events-manager').'</a>'; // by jwn

        ?>
		<form id="posts-filter" action="" method="get">
		<!--by jwn	<div class="subsubsub">
			...
			<p class="search-box">
				<label class="screen-reader-text" for="post-search-input"><?php _e('Search Doctors','events-manager'); ?>:</label>
				...
				<?php endif; ?>
				<input type="submit" value="<?php _e('Search Doctors','events-manager'); ?>" class="button" />
			</p>
			<div class="tablenav">
				<?php
				...
				<thead>
					<tr>
						...
						*/ ?>
						...
						<th>&nbsp;</th>
						<th nowrap><?php _e ( 'Institution', 'events-manager'); ?></th>
				...
				<tbody>
					...
					foreach ( $EM_Events as $EM_Event ) {
						...
						<tr class="event <?php echo trim($class); ?>" <?php echo $style; ?> id="event_<?php echo $EM_Event->event_id ?>">
							...
							*/ ?>
							<td>
                                ...
                                if(current_user_can('edit_users')) {// by jwn change'create_users'
                                    ?>
						...
						<!--by jwn	<td>
								...
								?>
							</td> to hide description for booking -->
						</tr>
						<?php
					}
					?>
