...
<?php
	...
	//by jwn if(!empty($show_add_new) && current_user_can('edit_locations')) echo '<a class="em-button button add-new-h2" href="'.em_add_get_params($_SERVER['REQUEST_URI'],array('action'=>'edit','scope'=>null,'status'=>null,'location_id'=>null)).'">'.__('Add New','events-manager').'</a>';
if(!empty($show_add_new) && current_user_can('edit_locations')) echo '<a class="em-button button add-new-h2" href="">'.__('Add New','events-manager').'</a>';
		...
		<tbody>
			...
			<?php foreach ($locations as $EM_Location) : ?>
				...
				<tr class="<?php echo $class; ?>">
					...
					<!-- by jwn	<a href='<?php echo esc_url($EM_Location->get_edit_url()); ?>'><?php echo esc_html($EM_Location->location_name); ?></a>-->
                        <?php echo esc_html($EM_Location->location_name); ?><!--by add jwn-->
                        <?php else: ?>
