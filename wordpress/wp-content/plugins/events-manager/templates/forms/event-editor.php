...
//check that user can access this page
 if( (is_object($EM_Event) &&  !$EM_Event->can_manage('edit_events','edit_others_events')) ){ //by jwn
//if( is_object($EM_Event) && (!$EM_Event->can_manage('edit_events','edit_others_events') || !$EM_Event->can_edit() )){|| (is_object($EM_Event) && !$EM_Event->can_edit() )
...
$owner_role = wp_get_current_user()->roles[1];

if($owner_role=='author' && get_current_user_id()!=$EM_Event->event_person ){
    ?>
    <div class="wrap"><h2><?php esc_html_e('Unauthorized Access','events-manager'); ?></h2><p><?php echo sprintf(__('You do not have the rights to manage this %s.','events-manager'),__('Event','events-manager')); ?></p></div>
    <?php
    return false;
...
<form enctype='multipart/form-data' id="event-form" class="em-event-admin-editor <?php if( $EM_Event->is_recurring() ) echo 'em-event-admin-recurring' ?>" method="post" action="<?php echo esc_url(add_query_arg(array('success'=>null))); ?>">
	<div class="wrap">
		...
		<?php endif; ?>
		<h3 class="event-form-name"><?php esc_html_e( 'Doctor Name', 'events-manager'); ?></h3>
		<div class="inside event-form-name">
			...
			<br />
			<?php esc_html_e('The doctor name. Example: Amitabh Harivansh Rai Shrivastava Bachchan ', 'events-manager'); ?>
			<?php em_locate_template('forms/event/group.php',true); ?>
		</div>
        <!--by jwn add--->

        <div class="inside event-form-name">
            <h3 class="event-form-mail"><?php esc_html_e( 'Email', 'events-manager'); ?></h3>
            <input type="text" name="event_email" id="event_email" value="<?php echo esc_attr($EM_Event->event_email,ENT_QUOTES); ?>" /><?php echo $required; ?>

            <h3 class="event-form-pass"><?php esc_html_e( 'Password', 'events-manager'); ?></h3>
            <input type="text" name="event_pass" id="event_pass" value="<?php echo esc_attr($EM_Event->event_pass,ENT_QUOTES); ?>" /><?php echo $required; ?>
        </div><!--by jwn end--->
        ...
        */
		...
		<h3><?php esc_html_e( 'Doctor Photo', 'events-manager'); ?></h3>
	...
	<p class="submit">
	    ...
	    <input type='submit' class='button-primary' value='<?php echo esc_attr(sprintf( __('Add %s','events-manager'), __('Doctor','events-manager') )); ?>' />
	    <?php else: ?>
	    <input type='submit' class='button-primary' value='<?php echo esc_attr(sprintf( __('Update %s','events-manager'), __('Doctor','events-manager') )); ?>' />
	    <?php endif; ?>
	</p>
