...
class EM_Event extends EM_Object{
	...
	var $event_owner;
	var $event_name;
	var $event_email;//by jwn;
    var $event_pass;//by jwn
    var $event_person;//by jwn
	...
	var $fields = array(
        ...
        'event_email' => array( 'name'=>'event_email', 'type'=>'%s', 'null'=>true ),//jwn
        'event_pass' => array( 'name'=>'event_pass', 'type'=>'%s', 'null'=>true ),//jwn
        'event_person' => array('name'=>'event_person', 'type'=>'%d', 'null'=>true),//jwn
	...
	function load_postdata($event_post, $search_by = false){
		...
		if( is_object($event_post) && ($event_post->post_type == 'event-recurring' || $event_post->post_type == EM_POST_TYPE_EVENT) ){
            ...
            $this->event_email= $event_post->post_email;//by jwn
            $this->event_pass = $event_post->post_pass;//by jwn
	...
	function get_post_meta(){
        ...
        $this->event_email = !empty($_POST['event_email']) ? htmlspecialchars_decode(wp_kses_data(htmlspecialchars_decode(wp_unslash($_POST['event_email'])))):'';//by jwn
        $this->event_pass = !empty($_POST['event_pass']) ? htmlspecialchars_decode(wp_kses_data(htmlspecialchars_decode(wp_unslash($_POST['event_pass'])))):'';//by jwn
			...
			if( !$this->is_recurring() ){
				if( get_option('dbem_bookings_tickets_single') && count($this->get_tickets()->tickets) == 1 ){
			    	...
			    	if( !empty($EM_Ticket->end_timestamp) ){
			    		...
			    		$this->event_rsvp_date = $this->event_end_date;
	...
	function delete($force_delete = false){ //atm wp seems to force cp deletions anyway
        ...
        um_fetch_user( $this->event_person );
	...
	function delete_events(){
		global $wpdb;global $ultimatemember;
