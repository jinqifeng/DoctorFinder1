...
class EM_Booking extends EM_Object{
	...
	function set_status($status, $email = true, $ignore_spaces = false){
		...
		$user_role = wp_get_current_user()->roles[1];
		if($user_role=='author' && $this->get_event()->event_person != get_current_user_id() ){
            $this->add_error(__('You can not access .','events-manager'));
            $this->feedback_message .= ' <span style="color:firebrick">'.__('You can not access.','events-manager').'</span>';
		...
		if($result !== false){
			...
			if( $email ){
				if( $this->email() ){
					...
					//$this->add_error(__('ERROR : Email Not Sent.','events-manager')); by jwn
				}
			}
		}else{
