...
function em_bookings_events_table() {
    ...
    if($owner_role=='author'){

        $event_array = $wpdb->get_row($wpdb->prepare("SELECT * FROM ".EM_EVENTS_TABLE." WHERE event_person=%d",get_current_user_id()), ARRAY_A);
        $EM_Event = em_get_event($event_array['event_id']);
        em_bookings_event();
        return;
    }
	...
	//	'past' => __ ( 'Past events', 'events-manager'),
		'all' => __ ( 'All doctors', 'events-manager'),
		'future' => __ ( 'Future doctors', 'events-manager')
	);
	...
	<div class="wrap em_bookings_events_table em_obj">
		<form id="posts-filter" action="" method="get">
			...
			<table class="widefat">
				<thead>
					<tr>
						<th><?php esc_html_e( 'Doctor', 'events-manager'); ?></th>
