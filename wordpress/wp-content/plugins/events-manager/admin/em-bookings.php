...
function em_bookings_dashboard(){
	...
	<div class='wrap em-bookings-dashboard'>
		<?php if( is_admin() ): ?>
  		...
  		<?php endif; ?>
  	<!--by  jwn	<div class="em-bookings-recent">
  		...
  		<div class="em-bookings-events">
			<h2><?php esc_html_e('Doctors With Bookings Enabled','events-manager'); ?></h2>
	...
	<div class='wrap'>
		<?php if( is_admin() ): ?><h1><?php else: ?><h2><?php endif; ?>
                <!--	</*?php echo sprintf(__('Manage %s Bookings', 'events-manager'), "'{$EM_Event->event_name}'"); ?>
                           </*?php $uid = $owner_role = wp_get_current_user()->roles[1];// by jwn
           //   if($uid!='author'){ ?>
  			  <a href="</*?php echo $EM_Event->get_permalink(); ?>" class="</*?php echo $header_button_classes; ?>"></*?php echo sprintf(__('View %s','events-manager'), __('Doctor', 'events-manager')) ?></a>
  			  <a href="</*?php echo $EM_Event->get_edit_url(); ?>" class="</*?php echo $header_button_classes; ?>"></*?php echo sprintf(__('Edit %s','events-manager'), __('Doctor Work', 'events-manager')) ?></a>
              </*?php } ?>
		...
		<div>
			<p><strong><?php esc_html_e('Doctor Name','events-manager'); ?></strong> : <?php echo esc_html($EM_Event->event_name); ?></p>
			<p>
			...
			<p>
				<strong><?php esc_html_e('Institution','events-manager'); ?></strong> :
				<!--<a class="row-title" href="</*?php echo admin_url(); ?>post.php?action=edit&amp;post=</*?php echo $EM_Event->get_location()->post_id ?>"></*?php echo ($EM_Event->get_location()->location_name); ?></a>by jwn-->
                <a class="row-title" ><?php echo ($EM_Event->get_location()->location_name); ?></a>
            </p>
		</div>
...
function em_bookings_person(){	
	...
	<!--	<h2><?php esc_html_e('Past And Present Bookings','events-manager'); ?></h2> by jwn -->
		<?php
		$EM_Bookings_Table = new EM_Bookings_Table();
		$EM_Bookings_Table->status = 'all';
